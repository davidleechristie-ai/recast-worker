(()=>{
  ...
  if(location.pathname==='/'||location.pathname==='/index.html'){
    ...
    if(nav&&!nav.querySelector('[href="/proof"]')){const proof=document.createElement('a');proof.className='navlink';proof.href='/proof';proof.textContent='Live evidence';nav.prepend(proof);}
    if(nav&&!nav.querySelector('[href="/dashboard-demo"]')){const a=document.createElement('a');a.className='navlink';a.href='/dashboard-demo';a.textContent='Dashboard demo';nav.prepend(a);}
  }
  send('page_view');
  document.addEventListener('click',e=>{
    ...
    if(href.startsWith('https://buy.stripe.com/')){send('checkout_start',{label,plan:'founding_149',source:'stripe_payment_link'});return;}
