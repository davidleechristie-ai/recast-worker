(()=>{
 ...
 function apply(){const top=qs('.resultsTop');if(!top)return;const quotes=read(),count=Math.max(1,quotes.length),compare=sessionStorage.getItem('hqc_journey_mode')==='compare'||sessionStorage.getItem('hqc_compare_intent')==='1';
  ...
  summary.innerHTML=count===1?`<div style="font-size:12px;font-weight:800;color:#2f6c5a;text-transform:uppercase">Quote checked</div><h2 style="margin:5px 0 8px">Here’s what to do next</h2><p style="margin:0 0 14px;line-height:1.55">Review the important gaps below. You can send the suggested questions to your installer, or add another quote to compare your options.</p><div style="display:flex;gap:10px;flex-wrap:wrap"><button data-next="questions" style="padding:12px 16px;border:0;border-radius:9px;background:#087f5b;color:white;font-weight:800">See questions for my installer →</button><button data-next="compare" style="padding:12px 16px;border:1px solid #b8d5ca;border-radius:9px;background:white;color:#17352d;font-weight:800">Compare with another quote →</button></div>`:`<div style="font-size:12px;font-weight:800;color:#2f6c5a;text-transform:uppercase">${count} quotes compared</div><h2 style="margin:5px 0 8px">Your comparison is ready</h2><p style="margin:0 0 14px;line-height:1.55">Start with the key differences and missing information below. Then use the installer questions to resolve anything important before you choose.</p><button data-next="questions" style="padding:12px 16px;border:0;border-radius:9px;background:#087f5b;color:white;font-weight:800">See what I should ask next →</button>`;
