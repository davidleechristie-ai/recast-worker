(()=>{
  ...
  function styleHero(){
    ...
    if(wrap){
      ...
      wrap.style.setProperty('max-height',mobile?'420px':'560px','important');
      wrap.style.setProperty('aspect-ratio',mobile?'1.02 / 1':'1.12 / 1','important');
    }
    img.style.setProperty('display','block','important');
    img.style.setProperty('width','100%','important');
    img.style.setProperty('height','100%','important');
    img.style.setProperty('object-fit','cover','important');
    img.style.setProperty('object-position',mobile?'57% 43%':'58% 43%','important');
    img.style.setProperty('margin','0','important');
    img.alt='Heat-pump quote comparison showing price and heat-loss differences';
    const heading=copy.querySelector('h1,h2');
    if(heading){
      heading.style.setProperty('font-size',mobile?'clamp(38px,10vw,54px)':'clamp(48px,5vw,68px)','important');
      heading.style.setProperty('line-height','1.03','important');
      heading.style.setProperty('letter-spacing','-.035em','important');
    }
    const button=copy.querySelector('button.primary');
    if(button){button.style.setProperty('width',mobile?'100%':'min(430px,100%)','important');button.style.setProperty('min-height','54px','important');}
    return true;
  }
  ...
  function applyHomeOnce(){
    const copy=document.querySelector('.copy');
    const img=[...document.querySelectorAll('img')].find(x=>/homepage-graphic\.png/i.test(x.getAttribute('src')||''));
    if(!copy||!img)return false;
    if(copy.dataset.hqcApprovedHero!=='1'){
      copy.dataset.hqcApprovedHero='1';
      const heading=copy.querySelector('h1,h2');
      ...
      if(!document.getElementById('hqc-landing-proof')){
        const proof=document.createElement('div');proof.id='hqc-landing-proof';proof.style.cssText='margin:18px 0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;font-size:11px;line-height:1.35;color:#40516a';proof.innerHTML='<span>✓ No account required</span><span>✓ No personal details needed</span><span>✓ One screenshot is enough</span>';
        button?.insertAdjacentElement('beforebegin',proof);
      }
    }
    styleHero();
    return true;
  ...
  let tries=0;const homeTimer=setInterval(()=>{tries++;if(applyHomeOnce()||tries>20)clearInterval(homeTimer);},150);
  applyHomeOnce();dynamicTick();
  const observer=new MutationObserver(()=>{dynamicTick();if(!document.querySelector('.copy'))return;applyHomeOnce();});
  observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('resize',styleHero,{passive:true});
