(()=>{
  ...
  function optimiseLanding(){
    ...
    const h2=copy.querySelector('h2');const p=copy.querySelector('p');const button=copy.querySelector('button.primary');
    if(h2)h2.textContent='Got a heat-pump quote? Check what’s missing before you sign.';
    if(p)p.textContent='Upload your quote in seconds. We highlight what’s included, what’s missing and the questions worth asking before you commit.';
    if(button)button.textContent='Check my quote — free →';
    const proof=document.createElement('div');proof.id='hqc-landing-proof';proof.style.cssText='margin-top:12px;padding:12px 14px;border:1px solid #d9e9e3;border-radius:10px;background:#f4faf7;font-size:11px;line-height:1.5;color:#526277';
    proof.innerHTML='<b style="color:#16493d">Start with just one quote.</b> One clear phone screenshot is enough. No account, email, phone number or address is required.';button?.insertAdjacentElement('afterend',proof);
  }
  function optimiseUpload(){
    const card=[...document.querySelectorAll('.card')].find(x=>/Add your quotes/i.test(x.querySelector('h1')?.textContent||''));if(!card||document.getElementById('hqc-upload-help'))return;
    const intro=card.querySelector('h1')?.nextElementSibling;if(intro)intro.textContent='Start with one genuine installer quote. A screenshot or clear phone photo is enough for automatic extraction; you can add another quote after the first check.';
