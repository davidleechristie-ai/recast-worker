(()=>{
  ...
  function apply(){
    ...
    const kh=qs('h2,h3',key);if(kh){kh.textContent='What’s missing?';kh.style.setProperty('font-size','26px','important');kh.style.setProperty('line-height','1.2','important');}
    ...
    let next=qs('#hqc-next-step',key);if(!next){next=document.createElement('div');next.id='hqc-next-step';next.innerHTML='<strong>Before you commit</strong><p>Get these points confirmed in writing before paying a deposit.</p>';const btn=qsa('button',key).find(b=>/send these questions/i.test(b.textContent||''));if(btn)btn.insertAdjacentElement('beforebegin',next);else key.appendChild(next);}next.style.cssText='margin:18px 0 14px;padding:16px 18px;border-radius:10px;background:#f1f7f4;color:#173b31;font-size:15px;line-height:1.5';const np=qs('p',next);if(np)np.style.cssText='margin:4px 0 0';
    const keyButton=qsa('button',key).find(b=>/send these questions/i.test(b.textContent||''));if(keyButton){keyButton.textContent='Copy questions →';keyButton.style.setProperty('width','100%','important');keyButton.style.setProperty('min-height','52px','important');keyButton.style.setProperty('font-size','16px','important');}
    const qh=qs('h2,h3',questions);if(qh){qh.textContent='Questions to send';qh.style.setProperty('font-size','26px','important');qh.style.setProperty('line-height','1.2','important');}
    ...
    const completeness=findPanel(/^Quote evidence completeness$/i);if(completeness){completeness.style.setProperty('margin-top','18px','important');completeness.style.setProperty('font-size','15px','important');const h=qs('h2,h3',completeness);if(h)h.style.setProperty('font-size','22px','important');qsa('small',completeness).forEach(x=>x.style.setProperty('font-size','12px','important'));}
