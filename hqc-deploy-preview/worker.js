...
export default {
  async fetch(request,env) {
    ...
    if(incoming.pathname==='/health') return new Response(JSON.stringify({ok:true,service:'hqc-migration-preview',frontend:'cloudflare-assets',apiBase:API_BASE}),{headers:{'content-type':'application/json','cache-control':'no-store'}});
    ...
    headers.set('x-hqc-cloudflare-preview','1');
