...
export async function handleSolarBatteryAnalysisRequest(request) {
  ...
  const unpack = (q) => {
    if (!q?.extractedMedia) {
      return { ok: true, quoteText: typeof q === 'string' ? q : q?.quoteText, provenance: null };
    }
    const media = normaliseExtractedMedia(q.extractedMedia);
    return media.ok
      ? { ok: true, quoteText: media.quoteText, provenance: media.provenance }
      : media;
  };

  if (Array.isArray(body.quotes)) {
    if (body.quotes.length < 2) return json({ error: 'comparison_requires_two_quotes' }, 400);
    const inputs = body.quotes.map((q, i) => ({
      ...unpack(q),
      quoteId: q?.quoteId || `quote_${i + 1}`,
      technology: q?.technology || technology,
    }));
    const invalid = inputs.find(q => !q.ok);
    if (invalid) return json({ error: invalid.error }, 400);
    if (inputs.some(q => typeof q.quoteText !== 'string' || !q.quoteText.trim())) {
      return json({ error: 'quote_text_required' }, 400);
    }
    const result = compareSolarBatteryQuotes(
      inputs.map(q => ({ quoteId: q.quoteId, quoteText: q.quoteText, technology: q.technology })),
      technology
    );
  ...
  const result = analyseSolarBatteryQuote({
