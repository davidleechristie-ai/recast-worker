...
export function extractSolarBatteryEvidence(quoteText = '', technology = 'solar_battery') {
  ...
  const batteryMatch = text.match(/([A-Z][A-Za-z]+)\s+([A-Z]{1,5}\d{1,3})\s+battery(?:,)?\s*(\d+(?:\.\d+)?)\s*kWh\s+usable(?:\s+(?:storage|capacity))?|([A-Z][A-Za-z]+)\s+([A-Z]{1,5}\d{1,3}),?\s+usable\s+(?:storage|capacity)\s+(\d+(?:\.\d+)?)\s*kWh/i);
