/* Safe compact-mobile restoration: deposit-stage quote check | aspect-ratio:1.94/1
...
(()=>{
  ...
  function apply(){
    ...
    if(!hero||!hero.contains(img)||hero.dataset.hqcCompactMobile==='safe-v2')return;
    const left=direct(hero,copy),right=direct(hero,img);
    hero.dataset.hqcCompactMobile='safe-v2';
    ...
    if(h){h.textContent='Got a heat-pump quote? Check it before you pay a deposit.';h.style.setProperty('font-size','clamp(38px,10.5vw,52px)','important');h.style.setProperty('line-height','1.02','important');h.style.setProperty('letter-spacing','-.035em','important');h.style.setProperty('margin','6px 0 12px','important');h.style.setProperty('visibility','visible','important');}
    if(lead){lead.textContent='Upload the installer quote you already have. We’ll flag missing heat-loss, flow-temperature, radiator and scope evidence before you commit.';lead.style.setProperty('font-size','17px','important');lead.style.setProperty('line-height','1.45','important');lead.style.setProperty('margin','0 0 14px','important');}
    const proof=qs('#hqc-landing-proof');if(proof){proof.innerHTML='<span>✓ Independent quote check</span><span>✓ No account required</span><span>✓ Screenshot, photo or PDF</span>';proof.style.setProperty('grid-template-columns','1fr','important');proof.style.setProperty('gap','6px','important');proof.style.setProperty('margin','12px 0 14px','important');}
    ...
    if(btn){btn.textContent='Check my quote before I pay — free →';btn.style.setProperty('width','100%','important');btn.style.setProperty('max-width','none','important');btn.style.setProperty('min-height','58px','important');btn.style.setProperty('font-size','17px','important');}
