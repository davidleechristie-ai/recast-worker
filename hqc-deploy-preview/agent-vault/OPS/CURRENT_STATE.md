...
Updated: 2026-09-24 13:44 Europe/London
...
## Evidence refreshed
- Revenue: authoritative live Home Quote Check Stripe PaymentIntents refreshed; `data=[]`, `has_more=false`. Genuine revenue remains **£0 / £1,000 monthly, £0 / £100 validation, 0 paying customers**. Stripe Sigma aggregation is unavailable because this account has no Sigma plan; direct live PaymentIntent evidence remains authoritative and complete (`has_more=false`).
...
- Production health: last full site-consistency verification remains **GREEN** (`35979158224`). A newer custom-domain canary (`35994641139`) exposed a test timing defect in the default Heat Pump isolation assertion: it sampled the upstream hero immediately after response commit before `approved-home-layout.js` had applied the verified Heat Pump proposition. Static/domain/API verification passed and the private Solar preview checks passed before that assertion. Canary fix `b07e0eb2ce9fed5da0ccb8ef614f95ba17913f5c` now waits for the approved journey-choice render before asserting default Heat Pump copy; verification run `36000842107` is queued. This is a canary/test reliability issue, not evidence of a public production regression.
...
- Private rendered Solar preview remains non-public. Latest canary `35994641139` successfully rendered the Solar preview at both tested viewports and passed the NOT PUBLIC badge assertions before failing the subsequent default-Heat-Pump timing assertion.
...
4. Close canary reliability verification for `b07e0eb2` and keep Solar non-public until full private upload→analysis→results verification is green.
5. Verify technology propagation into durable funnel/checkout events end-to-end.
6. Begin Financial Assumptions Check only after trustworthy Solar customer ingestion is executable end-to-end.

## Expected revenue impact
No genuine-revenue advance this run. The run removed a false-negative canary condition that could otherwise block safe Solar validation and future releases. The direct first-purchase lever remains Heat Pump start→upload activation.

## Next actions
1. Inspect `36000842107`; only restore the canary gate to green when the rendered default Heat Pump assertion passes after the approved layout is present.
2. Continue the measured Heat Pump upload-activation experiment without overlapping conversion hypotheses.
3. Implement/test the private Solar extracted-media JSON bridge without enabling public Solar or allowing Heat Pump fallback.
4. Require end-to-end rendered private upload → Solar analysis → results verification before any public CTA.
5. Verify technology propagation into durable funnel/checkout events end-to-end.
6. Keep £4.99 pricing stable until materially more genuine analyses reach the paid boundary.

## Durable learning
No OPS/LEARNINGS.md update this run. The canary failure is implementation/test timing evidence, not a reusable customer-demand lesson.
