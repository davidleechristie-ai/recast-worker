...
Updated: 2026-09-20 15:52 Europe/London
...
- Durable funnel: fresh production endpoint access was attempted but unavailable in this execution context. Latest settled successful snapshot remains 55 genuine landings → 16 checker starts → 5 uploads → 5 genuine analyses. Extended cohort remains 4 genuine analyses → 0 multi-quote analyses → 4 Decision Cases → 0 installer-question actions → 2 share intents → 0 share opens → 0 recipient starts → 0 outbound/commercial clicks → 0 durable checkouts. Unavailable fresh evidence is not treated as zero.
...
- Production health: direct public homepage and growth endpoint fetches were attempted but unavailable in the web execution context this run. Latest settled health remains the prior successful production metrics workflow and guarded checkout release; fresh health is null rather than assumed.
...
4. Solar/Battery: repository inspection confirms anonymous technology instrumentation already accepts `solar_battery` and `battery`, so the reusable measurement substrate exists. Continue technology-specific extraction/evidence correctness behind the non-public gate; no CTA until correctness and rendered journey verification pass.
5. Instrumentation: retain anonymous aggregate technology dimensions and do not expand PII.

## Expected revenue impact
Keeping conversion UI stable avoids confounding a tiny post-fix sample. Concentrating execution on qualified quote-holders and intake completion increases the probability and speed of producing enough genuine analyses to exercise the working £4.99 checkout. Confirming the existing technology-segmented metrics substrate reduces implementation risk for Solar/Battery without destabilising the heat-pump journey.
