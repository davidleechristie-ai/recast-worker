...
Updated: 2026-09-20 22:41 Europe/London
...
- Production health: fresh direct health/growth evidence was unavailable in this execution context. Latest settled health remains the prior successful production metrics workflow and guarded checkout release; fresh health is null rather than assumed.
...
- Tracked roadmap: `STRATEGY/PURCHASE_ADVISER_ROADMAP.md`, now converted from an untracked checklist into explicit workstream statuses and evidence-backed completed steps.
...
- Roadmap status this run: WS1 IN PROGRESS; WS2 IN PROGRESS; WS3 NOT STARTED pending Worker routing/adapter boundary; WS5 NOT STARTED pending trustworthy extraction; WS7 IN PROGRESS through the existing Heat Pump Decision Pack; WS8/WS9 DEFERRED.
- This remains implementation foundation only: technology routing is not yet wired into `worker.js`, CI/preview/canary verified or deployed. Solar/Battery remains non-public.
...
4. Solar/Battery: routing contract and isolation tests exist. Next safe slice is wire the contract into Worker analysis proxy behind the non-public gate, run tests and preserve Heat Pump behaviour; then implement structured Solar extraction fixtures/adapter.
5. Instrumentation: existing anonymous technology dimensions are reusable; do not expand PII.

## Expected revenue impact
The roadmap is now executable rather than descriptive: completed routing work is distinguished from unverified/deployed capability, dependencies are explicit, and future runs can resume at the first incomplete gate instead of rediscovering strategy. The routing contract reduces the risk that a Solar/Battery customer receives heat-pump analysis and shortens the route to a trustworthy second vertical. Acquisition remains the immediate route to first revenue.
