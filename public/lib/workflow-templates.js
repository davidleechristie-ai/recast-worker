...
function use(t,save){
 ...
 if(save&&window.RecastWorkflowLibrary){window.RecastWorkflowLibrary.save(d);window.showToastSafe?.('Template saved to Workflow Library');window.RecastHomeDepth?.activate('library',true);return;}
 if(window.RecastRecipeBuilder2){if(t.sample&&$('input')){$('input').value=t.sample;$('input').dispatchEvent(new Event('input',{bubbles:true}));}window.RecastHomeDepth?.activate('builder',false);window.RecastRecipeBuilder2.openWithDefinition(d);window.RecastFunnel?.track('template_sample_loaded',{template_id:t.id});document.getElementById('exploreRecast')?.scrollIntoView({behavior:'smooth',block:'start'});}
}
function render(){
