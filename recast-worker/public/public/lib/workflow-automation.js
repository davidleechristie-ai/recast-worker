...
(function(){
  ...
  let pendingHostedAction=null;
  let hostedPlan=null;

  async function readHostedPlan(){
    if(!token())return null;
    try{const d=await api('/v1/workflows/usage');return d.plan||null;}catch(_){return null;}
  }
  function closeHostedDeploy(){const m=$('hostedDeployModal');if(m)m.hidden=true;pendingHostedAction=null;}
  async function openHostedDeploy(item,intent){
    if(!item)return null;
    if(!preflight(item))return null;
    if(item.deploymentId){
      if(intent==='automation')return openAutomationSetup(item);
      toast('This workflow is already deployed.');return item;
    }
    pendingHostedAction={item,intent:intent||'deploy'};
    const m=$('hostedDeployModal'),confirmBtn=$('hostedDeployConfirm'),planText=$('hostedCurrentPlan'),plans=$('hostedDeployPlans');
    if(!m)return null;
    m.hidden=false;
    window.RecastFunnel?.track('hosted_disclosure_viewed',{intent:pendingHostedAction.intent,workflow_name:item.name||'Untitled'});
    hostedPlan=await readHostedPlan();
    const hasToken=!!token(),isAuto=/^automation_/.test(hostedPlan||''),isApi=/^(api|automation)_/.test(hostedPlan||'');
    if(planText){
      if(!hasToken)planText.innerHTML='<strong>No hosted plan detected.</strong> Your workflow remains local until you choose API or Automation access.';
      else if(isAuto)planText.innerHTML='<strong>Automation access active.</strong> You can deploy this workflow and configure a schedule.';
      else if(isApi)planText.innerHTML='<strong>API access active.</strong> You can deploy and run this workflow on demand. Scheduling requires Automation.';
      else planText.innerHTML='<strong>Your current plan does not include hosted workflows.</strong> Compare API and Automation before deploying.';
    }
    if(confirmBtn){
      confirmBtn.disabled=!isApi;
      confirmBtn.textContent=!isApi?'Hosted plan required':pendingHostedAction.intent==='automation'&&!isAuto?'Deploy API workflow →':'Deploy workflow →';
    }
    if(plans)plans.textContent=isApi&&isAuto?'View plan details':isApi?'Add Automation':'Compare plans';
  }
  async function performDeploy(item){
    ...
    try{
      ...
      window.RecastFunnel?.track('hosted_deploy_confirmed',{workflow_name:item.name||'Untitled',intent:pendingHostedAction?.intent||'deploy'});
      toast('Workflow deployed — API endpoint is ready.'); render(); return updated;
    }catch(e){toast(e.message);return null;}
  }
  async function confirmHostedDeploy(){
    const pending=pendingHostedAction;if(!pending)return;
    const intent=pending.intent,isAuto=/^automation_/.test(hostedPlan||'');
    const updated=await performDeploy(pending.item);
    closeHostedDeploy();
    if(!updated)return;
    if(intent==='automation'){
      if(isAuto)openAutomationSetup(updated);
      else showUpgrade('automation','Your API endpoint is ready. Scheduling requires the Automation plan.');
    }
  }
  async function deploy(item){return openHostedDeploy(item,'deploy');}
  ...
  function openAutomationSetup(item){
  ...
  async function schedule(item){
    ...
    if(!item)return;
    if(!item.deploymentId){await openHostedDeploy(item,'automation');return;}
    if(!requireToken())return;
    hostedPlan=await readHostedPlan();
    if(!/^automation_/.test(hostedPlan||'')){showUpgrade('automation','This workflow is API-ready. Scheduling, encrypted connectors and unattended runs require Automation.');return;}
    openAutomationSetup(item);
  }
  ...
  function render(){
    ...
    if(!items.length){wrap.innerHTML='<div class="auto-empty auto-empty-action"><div><small>NO WORKFLOW TO HOST YET</small><strong>Prove it locally first.</strong><span>Start from a template, test the result in your browser, then come back here only when the workflow is worth deploying.</span></div><button class="btn primary" data-auto-empty-template type="button">Try a workflow →</button></div>';wrap.querySelector('[data-auto-empty-template]')?.addEventListener('click',()=>window.RecastHomeDepth?.activate('templates',false));return;}
  ...
  function init(){render();refreshMetrics();syncHosted();refreshHealth();$('healthRefresh')?.addEventListener('click',refreshHealth);$('hostedDeployClose')?.addEventListener('click',closeHostedDeploy);$('hostedDeployCancel')?.addEventListener('click',closeHostedDeploy);$('hostedDeployConfirm')?.addEventListener('click',confirmHostedDeploy);$('hostedDeployPlans')?.addEventListener('click',()=>{window.RecastFunnel?.track('hosted_plan_compare_clicked',{intent:pendingHostedAction?.intent||'deploy'});closeHostedDeploy();});$('manageCredentialsBtn')?.addEventListener('click',openCredentialVault);$('credentialVaultClose')?.addEventListener('click',closeCredentialVault);$('credSave')?.addEventListener('click',saveCredential);$('credType')?.addEventListener('change',()=>{$('credHeaderWrap').style.display=$('credType').value==='api_key'?'grid':'none';});$('credentialVaultList')?.addEventListener('click',e=>{const b=e.target.closest('[data-delete-cred]');if(b)deleteCredential(b.dataset.deleteCred);});}
