...
(function () {
  ...
  $('rb2AutomateBtn')?.addEventListener('click', () => {
    ...
    if(window.RecastWorkflowLibrary){window.RecastWorkflowLibrary.save(current);window.RecastFunnel?.track('builder_automate_clicked',{workflow_name:name,step_count:current.steps.length});showToastSafe('Workflow saved — choose Automate to schedule it');window.RecastHomeDepth?.activate('automation',true);setTimeout(()=>window.RecastWorkflowAutomation?.render(),100);}
  });
