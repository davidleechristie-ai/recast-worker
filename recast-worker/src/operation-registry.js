...
function describeStep(step){ const op=getOperation(step?.mode); return op ? {mode:step.mode,label:op.label,description:op.description,category:op.category,params:step.params||{}} : null; }
function normalizePipeline(def={}){ const steps=Array.isArray(def.steps)?def.steps.map((step,index)=>({id:step.id||`step-${index+1}`,...describeStep(step)})):[]; return {version:2,goal:def.goal||'',input:def.input||{type:'unknown'},steps}; }
function validatePipeline(def={}){ const pipeline=normalizePipeline(def); if(!pipeline.steps.length) throw new Error('pipeline must contain at least one step'); for(const [index,step] of pipeline.steps.entries()) if(!getOperation(step.mode)) throw new Error(`unsupported pipeline step ${index+1}: ${step.mode||'missing mode'}`); return pipeline; }
